...
- [ ] 4.3 Прогнать `fmix check --stage all`; убедиться, что `flint lint . --strict --project-only` без предупреждений. Состояние: `fmix test` зелёный; flint видит 1 дубль в исходниках (`soc-top-out@`, уходит в `design-out-contract` 2.1) и 12 в сгенерированных `projects/*/csr.4th` (уходят в `iomap` 2.2, выход становится `csr.fs`). До тех пор коммиты через `FMIX_CHECK_NO_FLINT=1`.
