...
### Requirement: Отчёт сборки понятен без имён инструментов
Успешный `fsoc --build` в `projects/soc_emul` MUST напечатать, что записан `top.v`, и MUST напечатать, сколько байт из 8192 занял словарь ядра и чему равен указатель кода. Предупреждения Gforth MUST остаться на экране. Строка `tdp` MUST NOT склеиваться с текстом предупреждения.

#### Scenario: Две строки вместо emit ok и tdp
- **WHEN** сборка ядра проходит успешно
- **THEN** в выводе есть `Verilog: top.v` и строка `SwapForth nucleus: dictionary at $` с байтами из 8192 и указателем кода, а предупреждения Gforth видны отдельно
